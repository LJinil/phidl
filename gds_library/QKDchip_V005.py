...
def taper_v2(wid_i = 1, wid_o=0.5, taper_length = 3,shift=0, layer = 0):
    """ Creates an taper"""
    t = np.linspace(0, taper_length, 2)
    upper_points_x = (t).tolist()
    upper_points_y = (wid_i/2-(wid_i/2-wid_o/2-shift/2)/taper_length*t).tolist()
    lower_points_x = (t).tolist()
    lower_points_y = (-wid_i/2+(wid_i/2-wid_o/2+shift/2)/taper_length*t).tolist()
    xpts = upper_points_x + lower_points_x[::-1]
    ypts = upper_points_y + lower_points_y[::-1]

    D = Device('taper')
    D.add_polygon(points = (xpts,ypts), layer = layer)    
    D.add_port(name = 'wgport1', midpoint = [0,0], width = wid_i, orientation = 180)
    D.add_port(name = 'wgport2', midpoint = [taper_length,shift/2], width = wid_o, orientation = 0)
    D.info['length'] = taper_length
    return D
...
def MMI22_HK(length=100, width = 0.5,port_width=0.5, taper_length=10, taper_width=1.2,MMI_Wg_gap=0.5):
    MMI22 = Device('MMI22_HK')
    
    
    a= taper_length
    b= taper_length+length
    c= 2*taper_length+length
    port_pos= MMI_Wg_gap/2 + taper_width/2
    #Left port
    MMI22.add_polygon( [(0, port_pos-port_width/2), (a, port_pos-taper_width/2), (a, port_pos+taper_width/2), (0, port_pos+port_width/2)] )
    MMI22.add_polygon( [(0, -port_pos-port_width/2), (a, -port_pos-taper_width/2), (a, -port_pos+taper_width/2), (0, -port_pos+port_width/2)] )
    #body
...
def Beam_Splitter_DC(WGwidth = 1, coupling_length=100, coupling_gap = 0.5, port_gap12 = 301.1, port_gap34 = 301.1, buffer_length = 1000, a=0, b=0,ref_Length=1, layer = 0):
    ...
    for i in [1,-1]: # one for top one for bottom
        ...
        D.add_ref( pr.route_smooth(port1 = wg_CL.ports['wgport1'], port2 = wg_L_port.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
        er2=D.add_ref( pr.route_smooth(port1 = wg_R_port.ports['wgport1'], port2 = wg_CL.ports['wgport2'], path_type = 'sine', width_type = 'sine'))
...
def Beam_Splitter_MMI12(WGwidth = 1, taper_length=2, taper_width=1.2, MMI_coupling_length=100, MMI_coupling_width = 0.5,coupling_port_gap=0.5, port_gap34 = 301.1, buffer_length1 = 100, buffer_length = 1000, a=0, b=0,ref_Length=1, layer = 0):
    ...
    D.add_ref( pr.route_smooth(port1 = MMI.ports['wgport1'], port2 = wg_L_port1.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    R1_c=D.add_ref( pr.route_smooth(port1 = wg_R_port1.ports['wgport1'], port2 = MMI.ports['wgport2'], path_type = 'sine', width_type = 'sine'))
    R2_c=D.add_ref( pr.route_smooth(port1 = wg_R_port2.ports['wgport1'], port2 = MMI.ports['wgport3'], path_type = 'sine', width_type = 'sine'))
...
def Beam_Splitter_MMI22(WGwidth = 1, taper_length=10, taper_width=1.2, MMI_coupling_length=100, MMI_coupling_width = 0.5,port_gap_ratio=0.5, port_gap12 = 301.1, port_gap34 = 301.1, buffer_length = 1000, buffer_length2 = 1000, a=0, b=0,ref_Length=1, layer = 0):
    D = Device('Beam spliter with DC')
    ...
    MMI = D.add_ref(MMI22(length=MMI_coupling_length, width = MMI_coupling_width, taper_length=taper_length,port_gap_ratio=port_gap_ratio, taper_width=taper_width,port_width=WGwidth)) #coupling region 
    MMI.y= a
    ...
    D.add_ref( pr.route_smooth(port1 = MMI.ports['wgport1'], port2 = wg_L_port1.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    D.add_ref( pr.route_smooth(port1 = MMI.ports['wgport2'], port2 = wg_L_port2.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    er1=D.add_ref( pr.route_smooth(port1 = wg_R_port1.ports['wgport1'], port2 = MMI.ports['wgport3'], path_type = 'sine', width_type = 'sine'))
    er2=D.add_ref( pr.route_smooth(port1 = wg_R_port2.ports['wgport1'], port2 = MMI.ports['wgport4'], path_type = 'sine', width_type = 'sine'))
...
# def Beam_Splitter_MMI22_HK_Sbend(WGwidth=1, taper_length=10, taper_width=1.3, 
#                              MMI_coupling_length=100, MMI_coupling_width=0.5, 
#                              MMI_Wg_gap=0.5, port_gap12=301.1, port_gap34=301.1, 
#                              buffer_length=1000, buffer_length2=1000, a=0, b=0, 
#                              ref_Length=1, layer=0):
#     D = Device('Beam spliter with DC')
...
#     MMI = D.add_ref(MMI22_HK(length=MMI_coupling_length, width=MMI_coupling_width, 
#                              taper_length=taper_length, MMI_Wg_gap=MMI_Wg_gap, 
#                              taper_width=taper_width, port_width=WGwidth))  # coupling region 
#     MMI.y = a
#     MMI.xmin = wg_L_port1.xmax + buffer_length
...
#     # s-bend 형태의 경로 생성: path_type과 width_type을 's-bend'로 지정합니다.
#     D.add_ref(pr.route_smooth(port1=MMI.ports['wgport1'], 
#                              port2=wg_L_port1.ports['wgport2'], 
#                              path_type='s-bend', width_type='straight'))
#     D.add_ref(pr.route_smooth(port1=MMI.ports['wgport2'], 
#                              port2=wg_L_port2.ports['wgport2'], 
#                              path_type='s-bend', width_type='straight'))
#     er1 = D.add_ref(pr.route_smooth(port1=wg_R_port1.ports['wgport1'], 
#                                    port2=MMI.ports['wgport3'], 
#                                    path_type='s-bend', width_type='straight'))
#     er2 = D.add_ref(pr.route_smooth(port1=wg_R_port2.ports['wgport1'], 
#                                    port2=MMI.ports['wgport4'], 
#                                    path_type='s-bend', width_type='straight'))

#     length_differance = er1.info['length'] - er2.info['length']
#     D.add_port(name='wgport1', midpoint=[wg_L_port1.xmin, wg_L_port1.y], 
#                width=WGwidth, orientation=180)
#     D.add_port(name='wgport2', midpoint=[wg_L_port2.xmin, wg_L_port2.y], 
#                width=WGwidth, orientation=180)
#     D.add_port(name='wgport3', midpoint=[wg_R_port1.xmax, wg_R_port1.y], 
#                width=WGwidth, orientation=0)
#     D.add_port(name='wgport4', midpoint=[wg_R_port2.xmax, wg_R_port2.y], 
#                width=WGwidth, orientation=0)
#     D.info['length_differance'] = length_differance
...
def Beam_Splitter_MMI22_HK_Sbend(WGwidth=1, taper_length=10, taper_width=1.3, 
    ...
    D.add_ref(pr.route_smooth(port1=MMI.ports['wgport1'], 
                             port2=wg_L_port1.ports['wgport2'], 
                             path_type='manhattan', width_type='straight'))
    D.add_ref(pr.route_smooth(port1=MMI.ports['wgport2'], 
                             port2=wg_L_port2.ports['wgport2'], 
                             path_type='manhattan', width_type='straight'))
    er1 = D.add_ref(pr.route_smooth(port1=wg_R_port1.ports['wgport1'], 
...
def Beam_Splitter_MMI22_HK(WGwidth = 1, taper_length=10, taper_width=1.3, MMI_coupling_length=100, MMI_coupling_width = 0.5,MMI_Wg_gap=0.5, port_gap12 = 301.1, port_gap34 = 301.1, buffer_length = 1000, buffer_length2 = 1000, a=0, b=0,ref_Length=1, layer = 0):
    D = Device('Beam spliter with DC')
    ...
    MMI = D.add_ref(MMI22_HK(length=MMI_coupling_length, width = MMI_coupling_width, taper_length=taper_length,MMI_Wg_gap=MMI_Wg_gap, taper_width=taper_width,port_width=WGwidth)) #coupling region 
    MMI.y= a
    MMI.xmin = wg_L_port1.xmax+buffer_length
...
# ports and MMI cennection   
    ...
    D.add_ref(pr.route_smooth(port1 = wg_L_port2.ports['wgport2'], port2 = MMI1.ports['wgport1'], path_type = 'sine', width_type = 'sine')) 
    D.add_ref(pr.route_smooth(port1 = wg_L_port3.ports['wgport2'], port2 = MMI1.ports['wgport2'], path_type = 'sine', width_type = 'sine')) 
    ...
    D.add_ref(pr.route_smooth(port1 = wg_L_port1_1.ports['wgport2'], port2 = MMI2.ports['wgport1'], path_type = 'sine', width_type = 'sine')) 
    D.add_ref(pr.route_smooth(port1 = MMI1.ports['wgport3'], port2 = MMI2.ports['wgport2'], path_type = 'sine', width_type = 'sine')) 
        
    D.add_ref(pr.route_smooth(port1 = MMI1.ports['wgport4'], port2 = wg_R_port3_1.ports['wgport1'], path_type = 'sine', width_type = 'sine')) 
    D.add_ref(pr.route_smooth(port1 = MMI2.ports['wgport3'], port2 = wg_R_port1.ports['wgport1'], path_type = 'sine', width_type = 'sine')) 
    D.add_ref(pr.route_smooth(port1 = MMI2.ports['wgport4'], port2 = wg_R_port2.ports['wgport1'], path_type = 'sine', width_type = 'sine'))
...
# ports and MMI cennection   
    D.add_ref(pr.route_smooth(port1 = wg_L_port1.ports['wgport2'], port2 = MMI1.ports['wgport1'], path_type = 'sine', width_type = 'sine')) 
    D.add_ref(pr.route_smooth(port1 = wg_L_port2.ports['wgport2'], port2 = MMI1.ports['wgport2'], path_type = 'sine', width_type = 'sine')) 
    ...
    D.add_ref(pr.route_smooth(port1 = MMI1.ports['wgport3'], port2 = MMI2.ports['wgport1'], path_type = 'sine', width_type = 'sine')) 
 #    D.add_ref(pr.route_smooth(port1 = bend0.ports['wgport2'], port2 = wg_R_port1.ports['wgport2'], path_type = 'sine', width_type = 'sine')) 
    D.add_ref(pr.route_smooth(port1 = MMI2.ports['wgport4'], port2 = wg_R_port2.ports['wgport1'], path_type = 'sine', width_type = 'sine'))
 #   D.add_ref(pr.route_smooth(port1 = bend3.ports['wgport2'], port2 = wg_R_port3.ports['wgport1'], path_type = 'sine', width_type = 'sine')) 
 #   D.add_ref(pr.route_smooth(port1 = bend2.ports['wgport2'], port2 = wg_R_port4.ports['wgport1'], path_type = 'sine', width_type = 'sine'))
...
def rotate_BS_MMI22(WGwidth = 1, taper_length=10, taper_width=1.2, R_angle=30,R_bend=300, MMI_coupling_length=100, MMI_coupling_width = 0.5,coupling_port_gap=0.5, port_gap12 = 301.1, port_gap34 = 301.1, buffer_length = 1000, a=0, b=0,ref_Length=1, layer = 0):
    ...
    D.add_ref( pr.route_smooth(port1 = R1.ports['wgport1'], port2 = wg_L_port1.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    D.add_ref( pr.route_smooth(port1 = R2.ports['wgport1'], port2 = wg_L_port2.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    D.add_ref( pr.route_smooth(port1 = wg_R_port1.ports['wgport1'], port2 = R4.ports['wgport1'], path_type = 'sine', width_type = 'sine'))
    D.add_ref( pr.route_smooth(port1 = wg_R_port2.ports['wgport1'], port2 = R3.ports['wgport1'], path_type = 'sine', width_type = 'sine'))
...
def PSR_TEST(WGwidth = 0.6,Twidth1 = 1.5,Twidth2 = 2.5,Tlength1 = 1,Tlength2 = 2,Tlength3 = 1,outWGwidth1 = 1.1,outWGwidth2 = 1.4,port_angle=0.3,outlength = 2, layer = 0):
    ...
    D.add_ref(pr.route_smooth(port1 = outport1.ports['wgport2'], port2 = buffer1.ports['wgport2'], path_type = 'straight', width_type = 'sine'))
    D.add_ref(pr.route_smooth(port1 = outport2.ports['wgport2'], port2 = buffer2.ports['wgport2'], path_type = 'sine', width_type = 'sine'))
...
def Beam_Splitter_sym_MMI42(WGwidth = 1, taper_length=10, taper_width=1.2,radius=200, MMI_coupling_length=100, MMI_coupling_width = 0.5,coupling_port_gap=0.5, in_port_gap = 301.1, out_port_gap = 301.1, in_buffer_length = 10, out_buffer_length = 10, a=0, b=0,ref_Length=1, layer = 0):
    ...
    R1=D.add_ref( pr.route_smooth(port1 = MMI1.ports['wgport1'], port2 = wg_L_port2.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    R2=D.add_ref( pr.route_smooth(port1 = MMI1.ports['wgport2'], port2 = wg_L_port3.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    R1u=D.add_ref( pr.route_smooth(port1 = MMI1.ports['wgport1'], port2 = wg_L_port2.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    R1u.y=R1u.y+in_port_gap
    R2l=D.add_ref( pr.route_smooth(port1 = MMI1.ports['wgport2'], port2 = wg_L_port3.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    ...
    #D.add_ref( pr.route_smooth(port1 = MMI1u.ports['wgport1'], port2 = wg_L_port1.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    #D.add_ref( pr.route_smooth(port1 = MMI1d.ports['wgport1'], port2 = wg_L_port4.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  

    D.add_ref( pr.route_smooth(port1 = MMI2_1.ports['wgport1'], port2 = MMI1u.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    D.add_ref( pr.route_smooth(port1 = MMI2_1.ports['wgport2'], port2 = MMI1.ports['wgport3'], path_type = 'sine', width_type = 'sine'))  
    D.add_ref( pr.route_smooth(port1 = MMI2_2.ports['wgport1'], port2 = MMI1.ports['wgport4'], path_type = 'sine', width_type = 'sine'))  
    D.add_ref( pr.route_smooth(port1 = MMI2_2.ports['wgport2'], port2 = MMI1d.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  

    D.add_ref( pr.route_smooth(port1 = wg_R_port1.ports['wgport1'], port2 = MMI2_1.ports['wgport4'], path_type = 'sine', width_type = 'sine'))
    D.add_ref( pr.route_smooth(port1 = wg_R_port2.ports['wgport1'], port2 = MMI2_2.ports['wgport3'], path_type = 'sine', width_type = 'sine'))
...
def PSR_adiabatic(WGwidth = 1, WGport1s= 2.26, WGport1m1= 2.78, WGport1m2= 2.98, WGport1e= 3.5,WGport2s= 1.57, WGport2m1= 1.31, WGport2m2= 1.21 ,WGport2e= 0.95,WGT1=2,WGT2=1.6,conv_length=500,coupling_length=1600, coupling_gap = 0.6, port_gap12 = 301.1, Taper_buffer_length = 100,Taper_buffer_length2=100,Taper_buffer_length3=450, buffer_length = 1000, a=0,ref_Length=1, layer = 0):
    ...
    D.add_ref( pr.route_smooth(port1 = wg_CL1_1.ports['wgport1'], port2 = wg_L_port1.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    D.add_ref( pr.route_smooth(port1 = wg_CL2_1.ports['wgport1'], port2 = wg_L_port2.ports['wgport2'], path_type = 'sine', width_type = 'sine'))  
    D.add_ref( pr.route_smooth(port1 = wg_CL1_3.ports['wgport2'], port2 = wg_conv.ports['wgport1'], path_type = 'sine', width_type = 'sine'))
    D.add_ref( pr.route_smooth(port1 = wg_R_port.ports['wgport1'], port2 = wg_conv.ports['wgport2'], path_type = 'sine', width_type = 'sine'))
...
#  creat port 
    ...
    D1.add_ref(pr.route_smooth(port1 = L_input1.ports['wgport2'], port2 = O_A_DC.ports['wgport1'], path_type = 'sine', width_type = 'sine'))
    D1.add_ref(pr.route_smooth(port1 = L_input2.ports['wgport2'], port2 = O_A_DC.ports['wgport2'], path_type = 'sine', width_type = 'sine'))
    D1.add_ref(pr.route_smooth(port1 = R_input1.ports['wgport1'], port2 = O_A_DC.ports['wgport3'], path_type = 'sine', width_type = 'sine'))
    D1.add_ref(pr.route_smooth(port1 = R_input2.ports['wgport1'], port2 = O_A_DC.ports['wgport4'], path_type = 'sine', width_type = 'sine'))
...
#  creat port 
    ...
    R_input2.y=R_input1.y-port_gap34-b 
    R_input2.x=R_input1.x
    D1.add_ref(pr.route_smooth(port1 = L_input1.ports['wgport2'], port2 = L_A_DC.ports['wgport1'], path_type = 'sine', width_type = 'sine'))
    D1.add_ref(pr.route_smooth(port1 = L_input2.ports['wgport2'], port2 = L_A_DC.ports['wgport2'], path_type = 'sine', width_type = 'sine'))
    D1.add_ref(pr.route_smooth(port1 = R_input1.ports['wgport1'], port2 = L_A_DC.ports['wgport3'], path_type = 'sine', width_type = 'sine'))
    D1.add_ref(pr.route_smooth(port1 = R_input2.ports['wgport1'], port2 = L_A_DC.ports['wgport4'], path_type = 'sine', width_type = 'sine'))
...
#%%
def PSR_Rotator(w_i=0.65,w_o=3.1,w_1=1.75,w_2=2.35,l_1=200,l_2=500,l_3 = 200):
    """
    Parameters
    ----------
    w_i : input wg width. The default is 0.65.
    w_o : output wg width. The default is 3.1.
    w_1 : First Fast Taper output width. The default is 1.75.
    w_2 : Second Slow Taper output width. The default is 2.35.
    l_1 : First Fast Taper Length The default is 200.
    l_2 : Second Slow Taper Length The default is 500.
    l_3 : Third Fast Taper The default is 200.

    Returns
    -------
    D1 : Devce with ports
    """
    D1 = Device('PSR_Rotator')
    
    # 1st taper
    wg1=D1.add_ref(taper_v2(wid_i=w_i, wid_o=w_1 ,taper_length=l_1))

    # 2nd slow taper
    wg2=D1.add_ref(taper_v2(wid_i=w_1, wid_o=w_2,taper_length=l_2))
    wg2.connect(port = 'wgport1', destination = wg1.ports['wgport2'])
    
    # 3rd taper
    wg3=D1.add_ref(taper_v2(wid_i=w_2, wid_o=w_o ,taper_length=l_3))    
    wg3.connect(port = 'wgport1', destination = wg2.ports['wgport2'])
    # wg3.connect(port = 'wgport2', destination = wg2.ports['wgport1'])
    
    
    D1.add_port(name = 'wgport1', midpoint = [wg1.xmin,0], width = w_i, orientation = 180)
    D1.add_port(name = 'wgport2', midpoint = [wg3.xmax,0], width = w_o, orientation = 0)
        
    return D1


def PSR_Splitter(w_Bi=3,w_Ti=1,w_Bo=1.4,w_To=2.4,g=0.5,l=5000,p_Gap = 200,p_shift_x = 500,w_i=0.65,l_temp=100):
    """
    Parameters
    ----------
    w_Bi : Bottom Waveguide Input Width, The default is 3.
    w_Ti : Top Waveguide Input Width, The default is 1.
    w_Bo : Bottom Waveguide Output Width, The default is 2.4.
    w_To : Top Waveguide Output Width, The default is 1.4.
    g : Gap between Waveguides, The default is 0.5.
    l : Taper Length, The default is 5000.
    ...
    # 1st taper
    wg1=D1.add_ref(taper_v2(wid_i=w_Bi, wid_o=w_Bo,taper_length=l,shift=w_Bi-w_Bo))

    # 2nd slow taper
    wg2=D1.add_ref(taper_v2(wid_i=w_Ti, wid_o=w_To,taper_length=l,shift=-(w_Ti-w_To)))
    wg2.y = g + w_To/2 + w_Bi/2  
    
    
    
    #Between Output Waveguide2 & PSR output2
    wg3=D1.add_ref(taper_v2(wid_i = w_To, wid_o=w_i, taper_length = l_temp))
    wg3.ymax = wg2.y+p_Gap/2
    wg3.xmax = wg2.xmax + p_shift_x
    
    # Between Output Waveguide & PSR output 
    wg4=D1.add_ref(taper_v2(wid_i = w_Bo, wid_o=w_i, taper_length = l_temp))
    wg4.ymin = wg2.y-p_Gap/2
    wg4.xmax = wg2.xmax + p_shift_x
    
    
    D1.add_ref(pr.route_smooth(port1 = wg2.ports['wgport2'], port2 = wg3.ports['wgport1'], path_type = 'sine', width_type = 'straight'))
    D1.add_ref(pr.route_smooth(port1 = wg1.ports['wgport2'], port2 = wg4.ports['wgport1'], path_type = 'sine', width_type = 'straight'))
    ...
    D1.add_port(name = 'wgport1', midpoint = [wg1.xmin,wg1.ymax-w_Bi/2], width = w_Bi, orientation = 180)
    D1.add_port(name = 'wgport2', midpoint = [wg4.xmax,wg4.ymin+w_Bo/2], width = w_i, orientation = 0)
    D1.add_port(name = 'wgport3', midpoint = [wg3.xmax,wg3.ymax-w_To/2], width = w_i, orientation = 0)
